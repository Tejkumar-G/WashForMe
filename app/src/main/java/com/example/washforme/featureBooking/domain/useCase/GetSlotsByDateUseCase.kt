...
class GetSlotsByDateUseCase @Inject constructor(
    private val repo: BookingRepository
) {

    suspend operator fun invoke(date: String): ResponseData<List<PickUpSlotsModel>> {
        val response = repo.getPickUpSlotsByDate(date)
        return try {
