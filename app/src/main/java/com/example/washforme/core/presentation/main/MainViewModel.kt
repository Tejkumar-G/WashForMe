...
import com.example.washforme.core.domain.model.Category
...
////                        toast.postValue(it.payload.toString())
//                    }
//                    Status.FAILURE -> {
////                        isLoading.postValue(false)
    ...
    fun updateCountAndPrice() {
        ...
        _currentCart.value?.forEach { washCategoryRelation ->
            washCategoryRelation.items.forEach { item ->
                ...
                tempTotalPrice += item.count * item.price.toInt()
...
//        val userAddressFromPreference: UserAddress? =
...
//        }
    ...
    fun checkUserAlreadyLogIn(isUserLogin: (Boolean) -> Unit) {
        viewModelScope.launch {
            isUserLogin(checkUserLoginUseCase())
        }
    }
