...
class Repository @Inject constructor(
    ...
    suspend fun validateOTP(
        ...
        try {
            if (response.isSuccessful) {
                ...
                token?.let { preferences.set(PrefConstants.TOKEN, it) }
                tokenExpiry?.let { preferences.set(PrefConstants.EXPIRY, it) }
                result.value = ResponseData.success(true)
            } else {
                result.value = ResponseData.failure(response.errorBody().toString(), null)
            }
    ...
    suspend fun logoutUser(): MutableLiveData<ResponseData<Boolean>> {
        val result = MutableLiveData<ResponseData<Boolean>>()
        val response = api.logoutUser("Token ${preferences.getString(PrefConstants.TOKEN).toString()}")
        result.value = ResponseData.loading(null)
        try {
            if (response.isSuccessful) {
                preferences.clear()
    ...
    suspend fun getCategories(): Flow<ResponseData<WashCategoryResponseModel>> = flow {
    ...
    suspend fun getWashingItems(id: Int?= null): Flow<ResponseData<List<WashItem>>> = flow {
        emit(ResponseData.loading(null))
        try {
            val token = "Token ${preferences.getString(PrefConstants.TOKEN)}"
    ...
    suspend fun createUserWashRelation(categoryRelations: ArrayList<WashCategoryRelation>?): Flow<ResponseData<UserWashRelation>> = flow  {
        emit(ResponseData.loading(null))
        try {
            val response = api.createUserWashRelation("Token ${preferences.getString(PrefConstants.TOKEN).toString()}", categoryRelations)
    ...
    fun getUser(): Flow<ResponseData<User>> = flow {
        emit(ResponseData.loading(null))
        try {
            val response = api.getUserDetails("Token ${preferences.getString(PrefConstants.TOKEN)}")
    ...
    suspend fun getAllAddresses(): Flow<ResponseData<List<UserAddress>>> = flow {
        ...
        try {
            val response =
                api.getAllAddresses("Token ${preferences.getString(PrefConstants.TOKEN)}")
            if (response.isSuccessful) {
                response.body()?.let {
                    emit(ResponseData.success(it))
                }
    ...
    suspend fun changeCurrentAddress(id: String): Flow<ResponseData<UserAddress>> = flow {
        emit(ResponseData.loading(null))
        try {
            val response =
                api.changeCurrentAddress(id, "Token ${preferences.getString(PrefConstants.TOKEN)}")
            if (response.isSuccessful) {
                response.body()?.let {
                    emit(ResponseData.success(it))
                }
    ...
    suspend fun updateCurrentAddress(id: String, userAddress: UserAddress): Flow<ResponseData<UserAddress>> = flow {
        emit(ResponseData.loading(null))
        try {
            val response =
                api.updateCurrentAddress(id, userAddress, "Token ${preferences.getString(PrefConstants.TOKEN)}")
            if (response.isSuccessful) {
                response.body()?.let {
                    emit(ResponseData.success(it))
                }
    ...
    suspend fun createNewAddress(userAddress: UserAddress): Flow<ResponseData<UserAddress>> = flow {
        ...
        try {
            val response =
                api.createNewAddress(userAddress, "Token ${preferences.getString(PrefConstants.TOKEN)}")
            if (response.isSuccessful) {
                emit(ResponseData.success(response.body()))
            }
        } catch (e: HttpException) {
    ...
    suspend fun getPickUpSlots(): Flow<ResponseData<List<PickUpSlots>>> = flow {
        emit(ResponseData.loading(null))
        try {
            val response = api.getPickUpSlots("Token ${preferences.getString(PrefConstants.TOKEN)}")
            if (response.isSuccessful) {
                emit(ResponseData.success(response.body()))
            }
        } catch (e: HttpException) {
    ...
    suspend fun getPickUpSlotsByDate(date: String): Flow<ResponseData<List<PickUpSlots>>> = flow {
        emit(ResponseData.loading(null))
        try {
            val response = api.getPickUpSlotsByDate("Token ${preferences.getString(PrefConstants.TOKEN)}", date)
            if (response.isSuccessful) {
                emit(ResponseData.success(response.body()))
            }
        } catch (e: HttpException) {
